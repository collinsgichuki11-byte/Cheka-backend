...
// Apply the consecutive-creator diversity penalty in place: 2nd consecutive
// slot from the same creator gets 0.6x, 3rd+ get 0.3x. After penalising we
// resort, which effectively guarantees no creator appears more than twice
// in a row in the final list.
function applyDiversityPenalty(scored) {
  // Walk in current rank order; each time the same creator appears in a
  // consecutive run, attenuate its score.
  let prev = null;
  let run = 0;
  for (const v of scored) {
    ...
    if (cid === prev) {
      run++;
      if (run === 1) v._score *= DIVERSITY_PENALTY_2ND;        // 2nd in a row
      else v._score *= DIVERSITY_PENALTY_3RD;                  // 3rd+ in a row
    } else {
      run = 0;
      prev = cid;
    }
...
module.exports = {
  scoreVideo, rankVideos, baseScore, recencyDecay, applyDiversityPenalty,
